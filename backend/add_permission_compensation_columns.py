"""
Add compensation columns to crc6f_permissions via direct PostgreSQL connection.
Mirrors create_permission_table.py's connection-brute-force approach.

Usage:
    python add_permission_compensation_columns.py
"""
import os
import sys
import json
...
sql_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "add_permission_compensation_columns.sql")
...
for i, config in enumerate(connection_configs):
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute(SQL)
        print("[OK] crc6f_permissions compensation columns added successfully!")
