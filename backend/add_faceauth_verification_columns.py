"""
Add FaceAuth re-verification tracking columns to crc6f_table12s via direct
PostgreSQL connection. Mirrors add_permission_compensation_columns.py's
connection-brute-force approach.

Usage:
    python add_faceauth_verification_columns.py
"""
import os
import sys
import json
...
sql_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "add_faceauth_verification_columns.sql")
...
for i, config in enumerate(connection_configs):
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute(SQL)
        print("[OK] crc6f_table12s FaceAuth verification columns added successfully!")
